...
# version ='1.0'
# ---------------------------------------------------------------------------
# Programa para transformar puntos a coordenadas reais e extraer distancias
# ---------------------------------------------------------------------------
...
class PixelMapper(object):
    ...
    def calculate_speed(self,p1,p2):
        transformer = Transformer.from_crs(4326, 2062, always_xy=True) # Enforce lon,lat order
        # Usando distancia euclidea. Peor!!!
        #xy_1 = transformer.transform(p1[0], p1[1])
        #xy_2 = transformer.transform(p2[0], p2[1])
        #distance = np.sqrt((xy_1[0]-xy_2[0])**2+(xy_1[1]-xy_2[1])**2)

        # Usando fórmula de haversine
        distance = haversine(p1, p2, unit = Unit.METERS)
        return distance
